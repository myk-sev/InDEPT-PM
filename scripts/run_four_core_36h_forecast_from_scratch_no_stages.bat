...
rem Every epoch uses the full 36-hour objective; no horizon stages are used.
rem Usage: scripts\run_four_core_36h_forecast_from_scratch_no_stages.bat [--dry-run] [device] [batch_size] [workers]
if /i "%~1"=="--dry-run" (
    set "DRY_RUN=1"
    shift
)
...
echo Completed %TRAINING_COUNT% fresh core 36-hour no-stage training runs.
cd /d "%START_DIR%"
exit /b 0
...
set "ARTIFACT_NAME=all_excl_fine_t_%FORECAST_MODEL%-from-scratch-no-stages-36h"
...
echo Prediction window: 36 hours
echo Epochs: 50
echo Forecast curriculum: none; every epoch uses 36 hours
