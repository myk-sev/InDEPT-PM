...
const locationInput=document.getElementById('location'),locationList=document.getElementById('locations'),startInput=document.getElementById('start'),endInput=document.getElementById('end'),resetZoom=document.getElementById('resetZoom'),status=document.getElementById('status'),error=document.getElementById('error'),quality=document.getElementById('quality'),invalidLegend=document.getElementById('invalidLegend'),reading=document.getElementById('reading'),chart=document.getElementById('chart'),overlay=document.getElementById('overlay');
let current=null,pendingSensor=null,visible=[],geometry=null,overlayContext=null,selectedTime=null,pinned=false,zoomStart=null,zoomEnd=null,drag=null;
...
function loadLocation(location){if(current?.indoor_sensor_id===location.indoor_sensor_id)return;pendingSensor=location.indoor_sensor_id;current=null;visible=[];selectedTime=null;pinned=false;zoomStart=zoomEnd=null;resetZoom.disabled=true;showError();quality.hidden=true;invalidLegend.hidden=true;status.textContent=`Loading ${location.label}…`;locationInput.value=location.label;const script=document.createElement('script');script.src=`location_history_data/${location.indoor_sensor_id}.js`;script.onload=()=>script.remove();script.onerror=()=>{showError('The location history could not be loaded. Keep the location_history_data folder beside this file.');status.textContent='';script.remove()};document.head.append(script)}
...
locationInput.addEventListener('change',chooseLocation);locationInput.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();chooseLocation()}});document.getElementById('apply').onclick=()=>{zoomStart=zoomEnd=null;draw()};document.getElementById('clear').onclick=()=>{startInput.value='';endInput.value='';zoomStart=zoomEnd=null;draw()};resetZoom.onclick=()=>{zoomStart=zoomEnd=null;pinned=false;draw()};
