...
<style>
:root{color-scheme:light dark;--bg:#fff;--fg:#202124;--muted:#666;--line:#ccd2d8;--in:#2878b5;--out:#c43c39;--bad:#f1c75b55;--bad-line:#9a6800;--select:#2878b544;--select-line:#185b8d} @media(prefers-color-scheme:dark){:root{--bg:#17191c;--fg:#eee;--muted:#aaa;--line:#59616b;--in:#6db7e8;--out:#f08078;--bad:#b9873255;--bad-line:#f1c75b;--select:#6db7e844;--select-line:#9bd4f5}} *{box-sizing:border-box} body{margin:0 auto;max-width:1200px;padding:1rem;font:15px/1.45 system-ui,sans-serif;background:var(--bg);color:var(--fg)} h1{font-size:1.4rem;margin:.2rem 0}.pages{display:flex;gap:.4rem;margin:.8rem 0}.pages a{color:var(--fg);padding:.45rem .7rem;border:1px solid var(--line);border-radius:.25rem;text-decoration:none}.pages a[aria-current=page]{color:var(--bg);background:var(--fg)}.controls{display:flex;gap:.7rem;align-items:end;flex-wrap:wrap;margin:1rem 0}.controls label{display:grid;gap:.25rem}.location{flex:1 1 32rem}input,button{font:inherit;padding:.45rem .6rem}input[type=search]{width:100%}button:disabled{opacity:.55}.muted{color:var(--muted)}.error{color:#b3261e}.legend,.reading{display:flex;gap:1.2rem;flex-wrap:wrap;margin:.6rem 0;font-variant-numeric:tabular-nums}.legend span:before{content:"";display:inline-block;width:1.4rem;border-top:3px solid;margin-right:.35rem;vertical-align:middle}.legend .outdoor:before{border-color:var(--out)}.legend .indoor:before{border-color:var(--in)}.legend .invalid:before{border-color:var(--bad-line);border-top-width:8px}.reading .outdoor{color:var(--out)}.reading .indoor{color:var(--in)}.chart-shell{position:relative;height:560px}.chart-shell canvas{position:absolute;inset:0;width:100%;height:100%}#chart{border:1px solid var(--line)}#overlay{cursor:crosshair;touch-action:none}@media(max-width:600px){body{padding:.7rem}.chart-shell{height:480px}.controls>*{flex:1 1 100%}}
</style>
</head>
<body>
...
<div class="controls">
  ...
  <button id="previousExcluded" type="button" hidden aria-label="Previous excluded sensor">&larr; Previous sensor</button>
  <button id="nextExcluded" type="button" hidden aria-label="Next excluded sensor">Next sensor &rarr;</button>
</div>
...
function updateExcludedNavigation(){const visible=page==='excluded',index=locations.findIndex(location=>location.file===pendingLocation);previousExcluded.hidden=nextExcluded.hidden=!visible;previousExcluded.disabled=index<=0;nextExcluded.disabled=index<0||index>=locations.length-1}
function stepExcludedSensor(offset){const index=locations.findIndex(location=>location.file===pendingLocation),next=index+offset;if(page==='excluded'&&next>=0&&next<locations.length)loadLocation(locations[next])}
function loadLocation(location){if(current?.data_file===location.file)return;pendingLocation=location.file;updateExcludedNavigation();current=null;visible=[];selectedTime=null;pinned=false;zoomStart=zoomEnd=null;resetZoom.disabled=true;showError();quality.hidden=true;invalidLegend.hidden=true;status.textContent=`Loading ${location.label}…`;locationInput.value=location.label;const script=document.createElement('script');script.src=`location_history_data/${location.file}`;script.onload=()=>script.remove();script.onerror=()=>{showError('The location history could not be loaded. Keep the location_history_data folder beside this file.');status.textContent='';script.remove()};document.head.append(script)}
...
locationInput.addEventListener('change',chooseLocation);locationInput.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();chooseLocation()}});document.getElementById('apply').onclick=()=>{zoomStart=zoomEnd=null;draw()};document.getElementById('clear').onclick=()=>{startInput.value='';endInput.value='';zoomStart=zoomEnd=null;draw()};resetZoom.onclick=()=>{zoomStart=zoomEnd=null;pinned=false;draw()};previousExcluded.onclick=()=>stepExcludedSensor(-1);nextExcluded.onclick=()=>stepExcludedSensor(1);
...
function showPage(){const hash=window.location.hash,next=hash==='#excluded'?'excluded':hash==='#unpaired'?'unpaired':'paired';if(next===page)return;page=next;locations=page==='paired'?pairedLocations:page==='unpaired'?unpairedLocations:excludedLocations;current=null;pendingLocation=null;visible=[];geometry=null;locationInput.value='';locationList.replaceChildren();locations.forEach(location=>{const option=document.createElement('option');option.value=location.label;locationList.append(option)});const links={paired:pairedPage,unpaired:unpairedPage,excluded:excludedPage};Object.values(links).forEach(link=>link.removeAttribute('aria-current'));links[page].setAttribute('aria-current','page');locationLabel.textContent=page==='paired'?'Location':'Sensor';locationHelp.textContent=page==='paired'?'Search by location name or sensor ID. Blank dates show the entire available history. Drag across either chart and release to zoom; click without dragging to pin a reading.':page==='unpaired'?'Search by sensor ID. These sensors have downloaded indoor history but no current pair. Blank dates show the entire available history; drag to zoom or click to pin a reading.':'Search by sensor name or ID. Use the arrow buttons to review excluded sensors one at a time. Highlighted periods are excluded from analysis and training.';outdoorLegend.hidden=page==='unpaired';indoorLegend.hidden=false;quality.hidden=true;invalidLegend.hidden=true;startInput.value=endInput.value='';status.textContent='';const empty=page==='paired'?'No locations have both indoor and outdoor PurpleAir histories.':page==='unpaired'?'No unpaired sensors have downloaded indoor histories.':'No excluded sensors have downloaded histories.';reading.textContent=locations.length?'Select a sensor to inspect its readings.':empty;showError();updateExcludedNavigation();if(locations.length){locationInput.value=locations[0].label;loadLocation(locations[0])}else showError(empty)}
